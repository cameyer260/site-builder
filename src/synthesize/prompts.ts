...
export function buildProfilePrompt(input: {
  ...
  return [
    ...
    "Read every relevant Markdown file there (site pages, extracted documents, and",
    "notes.md). Here is the index of what was ingested:",
    manifestSummary(input.manifest),
    "",
    "Fill in this Checklist. For every item, decide a Field status:",
    "- Known: directly supported by the ingested material.",
    ...
    '  "fields": [ { "key": "<checklist key>", "label": "<label>", "value": <string, array of strings for lists, or null>, "status": "Known|Guessed|Unknown", "note": "<optional>" } ],',
    '  "contact": { "email": "<optional>", "phone": "<optional>", "address": "<optional>" }',
    "}",
    "Include one `fields` entry for EVERY checklist key above, in order.",
    "",
    "2. profile.md — a clear, human-readable Client Profile grouped by section,",
    "with each answer tagged by its status, e.g. `**Industry** (Known): …`.",
