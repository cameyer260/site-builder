 ...
 * slots, the `audit` Scorecard, the completion marker) instead of scattering
 * them at the project root or under a confusingly-named `generate/` folder that
 * reads like app code. Dot-prefixed so it reads immediately as tool metadata
 * (mirroring `.github/`, `.vscode/`) rather than a mystery top-level folder once
 * this prototype evolves into the Client's production repo.
 */
...
and \`audit\` — not part of the deployed website, and never imported by the
Astro app:
...
- \`lighthouse.json\` — the audit Scorecard: achieved Lighthouse scores
  (Performance, Accessibility, Best Practices, SEO) per form factor.
