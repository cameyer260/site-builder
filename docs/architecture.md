...
- **`generate`** (`src/generate/`, ADR-0005/0006) — derives the Design Brief
  (`brief.md`, brand-color extraction, `--vibe`/`--style`), copies the
  [Kit](kit.md) into `sites/vN/`, `git init`s it, has the engine build on top
  invoking the `ui-ux-pro-max` skill, sources imagery (`pexels.ts` three-tier
  rule), and gates on `astro build`. An optional [QA session](../CONTEXT.md)
